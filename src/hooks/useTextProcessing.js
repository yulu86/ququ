...
export const useTextProcessing = () => {
  ...
  const callAIAPI = useCallback(async (text, mode) => {
    ...
    const prompts = {
      ...
      optimize: `# 角色与目标
你是一个专业的语音转录文本优化助手，熟悉软件开发领域尤其是Godot的英文词汇。任务是对由ASR（自动语音识别）生成的初步文本进行精细的、最小化的润色。你的核心目标是去除言语组织过程中的干扰性噪音，同时100%保留说话人的原始意图、个人风格和口语习惯。
...
5.  **中英文混杂**： 对于中英文混杂的情况，尽量保持中英文的连贯性，尽量将英文单词转换原文
    -   例子1: "我希望你能够帮我使用go到的二d开发一个五子棋游戏" -> "我希望你能够帮我使用Godot 2D开发一个五子棋游戏"
6.  **专有词汇**：对于软件开发相关的专有词汇请保持原文，并修改正确的大小写
    -   例子1: "请使用area 2D节点" -> "请使用Area2D节点"
